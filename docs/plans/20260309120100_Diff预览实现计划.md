...
**Files:**
- Modify: `cCli/package.json`

**Step 1: 安装 diff 包和类型**

```bash
cd cCli && pnpm add diff && pnpm add -D @types/diff
```

**Step 2: 验证安装成功**

```bash
cd cCli && pnpm exec tsc --noEmit
```
Expected: 无报错
...
```bash
git add cCli/package.json cCli/pnpm-lock.yaml
...
**Files:**
- Create: `cCli/src/utils/compute-diff.ts`
- Create: `cCli/tests/unit/compute-diff.test.ts`
...
```bash
cd cCli && pnpm test -- tests/unit/compute-diff.test.ts
```
Expected: FAIL（模块不存在）
...
```bash
git add cCli/src/utils/compute-diff.ts cCli/tests/unit/compute-diff.test.ts
...
**Files:**
- Create: `cCli/src/ui/DiffView.tsx`
- Create: `cCli/tests/unit/diff-view.test.tsx`
...
```bash
cd cCli && pnpm test -- tests/unit/diff-view.test.tsx
```
Expected: FAIL
...
```bash
git add cCli/src/ui/DiffView.tsx cCli/tests/unit/diff-view.test.tsx
...
- Create: `cCli/tests/unit/permission-dialog-diff.test.tsx`
...
```bash
cd cCli && pnpm test -- tests/unit/permission-dialog-diff.test.tsx
```
Expected: FAIL（PermissionDialog 未渲染 diff）

**Step 3: 修改 PermissionDialog**

修改 `cCli/src/ui/PermissionDialog.tsx`，关键变更：
...
```bash
git add cCli/src/ui/PermissionDialog.tsx cCli/tests/unit/permission-dialog-diff.test.tsx
